...
int main() {
  ...
  while (!glfwWindowShouldClose(viewer.window)) {
    ...
    Eigen::Vector3d pos_trg( 1.5+sin(3*time), cos(2*time), 1*cos(time)+1.5 ); // target position
    {
      ...
      double alpha = 0.5;
      Eigen::Matrix<double,8,8> inv_hessian = (diff_pos_def.transpose()*diff_pos_def+1/alpha*Eigen::Matrix<double,8,8>::Identity()).inverse();
      Eigen::Matrix<double,8,1> angle1 = angle0 - inv_hessian*diff_pos_def.transpose()*(pos_def - pos_trg);
      arb.angle = angle1;
      // editing ends here
      arb.UpdateTransformations();
